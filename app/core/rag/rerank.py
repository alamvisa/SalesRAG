...
class ranker():
    def __init__(self):
        ...
        self.table_desc = {
            ...
            "agg_item_category":     "Sales broken down by product category (Technology, Furniture, Office Supplies)",
            ...
            "agg_month_x_category":  "Monthly sales broken down by product category, monthly data (Technology, Furniture, Office Supplies)",
            "agg_year_x_category":   "Annual sales broken down by product category, yearly data (Technology, Furniture, Office Supplies)",
            "agg_state_x_category":  "State-level sales broken down by product category (Technology, Furniture, Office Supplies)",
    ...
    def rank(self, query, chunks):
        ...
        scores = self.model.predict(q, show_progress_bar=False)
        ranked = zip(chunks, scores)
        ranked = sorted(ranked, key=lambda x: x[1], reverse=True)
        top = [c for c, s in ranked if s >= ranked[0][1]*0.1][:12]
        logger.info(json.dumps({
